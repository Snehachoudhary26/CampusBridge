      ...
      <div style={{
        ...
        {categories.map(c => (
          <button key={c.name} onClick={() => setActiveCategory(c.name)} style={{
            ...
            fontWeight: 600, fontSize: 14, cursor: "pointer", onClick: () => window.location.href=`/listings/${item.id}`,
      ...
      <div style={{ display: 'flex', maxWidth: 1280, margin: '0 auto', padding: '28px 24px', gap: 24 }}>
        ...
        >
          ...
          <div style={{ marginBottom: 24 }}>
            ...
            {listingTypes.map(t => (
              <button key={t} onClick={() => setActiveType(t)} style={{
                ...
                fontSize: 14, cursor: "pointer", onClick: () => window.location.href=`/listings/${item.id}`, transition: 'all 0.2s',
              }}>
              ...
              {[['Min ₹', minPrice, setMinPrice], ['Max ₹', maxPrice, setMaxPrice]].map(([ph, val, set]) => (
                <input key={ph} type="number" placeholder={ph} value={val}
                  ...
                  style={{
                    ...
                    fontSize: 13, color: '#0D2B35', background: '#F8FFFE',
                    width: 0,
                  }}
            ...
            <div style={{ display: 'flex', gap: 6 }}>
              {[1,2,3,4,5].map(n => (
                <button key={n} onClick={() => setMinCondition(minCondition === n ? null : n)} style={{
                  ...
                  fontWeight: 700, fontSize: 14, cursor: "pointer", onClick: () => window.location.href=`/listings/${item.id}`, transition: 'all 0.2s',
                }}>{n}</button>
              ))}
            </div>
          ...
          <button onClick={clearFilters} style={{
            ...
            color: '#00A896', fontWeight: 700, fontSize: 14, cursor: "pointer", onClick: () => window.location.href=`/listings/${item.id}`,
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20, gap: 16, flexWrap: 'wrap' }}>
            ...
            <input
              ...
              style={{
                ...
                border: '1.5px solid #D0ECE8', outline: 'none',
                fontSize: 14, color: '#0D2B35', background: '#fff',
                minWidth: 240, transition: 'border 0.2s',
              }}
              onFocus={e => e.target.style.borderColor = '#00C9B1'}
              onBlur={e => e.target.style.borderColor = '#D0ECE8'}
            />
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(240px,1fr))', gap: 20 }}>
              {[...Array(6)].map((_, i) => (
                <div key={i} style={{
                  ...
                  border: '1px solid #E0F5F0', height: 300,
                  animation: 'pulse 1.5s ease-in-out infinite',
                }}>
                  <div style={{ height: 180, background: 'linear-gradient(90deg, #F0FFFE, #E0FBF8, #F0FFFE)', backgroundSize: '200%' }} />
          ...
          {!loading && listings.length === 0 && (
            <div style={{
              ...
              background: '#fff', borderRadius: 20, border: '1px dashed #B2EFE8',
            }}>
              ...
              <div style={{ display: 'flex', gap: 12, justifyContent: 'center' }}>
                <button onClick={clearFilters} style={{
                  ...
                  color: '#00A896', background: '#fff', fontWeight: 600, cursor: "pointer", onClick: () => window.location.href=`/listings/${item.id}`,
              ...
              {listings.map((item, i) => {
                ...
                return (
                  <motion.div key={item.id}
                    ...
                    whileHover={{ y: -5, boxShadow: '0 12px 35px rgba(0,201,177,0.15)' }} onClick={() => window.location.href = '/listings/' + item.id}
                    style={{
                      background: '#fff', borderRadius: 18, overflow: 'hidden',
                      border: '1px solid #E0F5F0',
                      boxShadow: '0 2px 12px rgba(0,0,0,0.04)',
                      transition: "all 0.3s", cursor: "pointer", onClick: () => window.location.href=`/listings/${item.id}`,
                    }}
                  >
                    {item.image_url
                      ...
                      : <div style={{
                          height: 175, fontSize: 48,
                          background: 'linear-gradient(135deg, #E0FBF8, #D0F8F3)',
                    ...
                    <div style={{ padding: 18 }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
                        <span style={{
                          ...
                          background: tc.bg, color: tc.color, fontWeight: 700, letterSpacing: 0.5,
                        }}>{item.listing_type?.toUpperCase()}</span>
                        <span style={{ color: '#00A896', fontWeight: 800, fontSize: 17 }}>₹{item.price}</span>
                      </div>
                      <h3 style={{ fontWeight: 700, color: '#0D2B35', marginBottom: 6, fontSize: 15, lineHeight: 1.3 }}>
                        {item.title}
                      </h3>
                      <p style={{ color: '#7A9BA8', fontSize: 13 }}>
                        {item.category} · ⭐ {item.condition}/5
                      </p>
